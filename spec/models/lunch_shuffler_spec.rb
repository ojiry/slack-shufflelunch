...
RSpec.describe LunchShuffler, type: :model do
  ...
  describe '#shuffle!' do
    ...
    it { expect { lunch_shuffler.shuffle! }.to change { [Group.count, GroupMember.count] }.from([0, 0]).to([3, 10]) }
    ...
    context 'when lunch has #previous_lunch' do
      ...
      it { expect { lunch_shuffler.shuffle! }.to change { [Group.count, GroupMember.count] }.from([2, 6]).to([5, 16]) }
    end
  end
